...
class StateManager:
    """
    집중 상태를 관리하고, 상태가 전환되는 순간(State Transition)에
    FocusEventClient 를 통해 백엔드로 이벤트를 전송한다.
    ...
    상태 전환 → 이벤트 매핑:
        ...
        좋음  → 이탈    : AWAY_START
        이탈  → 좋음    : AWAY_END
        눈 감음 → 이탈  : DROWSINESS_END + AWAY_START  (교차 전환)
        이탈  → 눈 감음 : AWAY_END + DROWSINESS_START  (교차 전환)
    """
    ...
    def _on_state_changed(self) -> None:
        """상태가 바뀐 경우에만 실행 — 콘솔 출력 + 이벤트 전송."""
        if self.confirmed_status == self.last_logged_status:
            return

        prev, new = self.last_logged_status, self.confirmed_status
